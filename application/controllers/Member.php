...
class Member extends CI_Controller {
    ...
    public function guncelle($id){
        ...
        if ($validate){
            ...
            $update = $this->member_model->guncelle(
                array(
                    "id"    => $id
                ),
                array(
   ...
   public function datatables_data()
    {
        ...
        //burası
        $toplam_veri_sayisi = $this->member_model->toplam_veri_sayisi(); // gittigi tablo
        ...
        if (!empty($arama_kelime)) {
            ...
            $sql .= $arama_sql;
            $toplam_veri_sayisi = $this->member_model->arama_toplam_veri_sayisi($sql);
        }
        ...
        $veriler = $this->member_model->verileri_getir($sql);
        ...
        foreach ($veriler as $key => $veri) {
            ...
            $veriDizi[] = "<a href='" . base_url("member/view_form/{$veri->id}")."' class='btn btn-sm btn-purple btn-outline'><i class='fa fa-info'></i> Detaylar</a>";
            $veriDizi[] = "<a href='" . base_url("member/update_form/{$veri->id}")."' class='btn btn-sm btn-success btn-outline'><i class='fa fa-edit'></i> Düzenle</a>";
            $veriDizi[] = "<a data-url='" . base_url("member/sil/{$veri->id}")."'  class='btn btn-sm btn-danger btn-outline btn-delete' ><i class='fa fa-trash'></i> Sil</a>";
           // $veriDizi[] = "<button data-url='" . base_url("member/sil/{$veri->id}")."' id='sil-ulan' class='btn btn-sm btn-danger btn-outline delete'><i class='fa fa-trash'> Sil</i></a>";
        ...
        foreach($data as $key => $value) {
          if($data[$key][7] == 'Evet') {
            $data[$key][7] = '<span class="label label-danger">Evet</span>';
          }
        }

        $output = array(
            "draw"            => intval($draw),
            "recordsTotal"    => $this->member_model->toplam_veri_sayisi(),
